...
- children are positional, identification first: a sow reads *taproot ring, plot ring, face bloom*, then the taproot's child rings (2026-07-20: a taproot's face has grafts like any branch face, so sow carries the tail too — genesis found the gap); a shoot reads *card ring, face bloom*, then the card's **child rings in graft order** — the ordered list a back needs to resolve the face's grafts (rings cannot be inferred from adjacent log records, so the shoot carries them). the back projection is the last shoot per ring, verbatim: the back-record encoding phase 4 promised arrives here naturally
- **child rings pair 1:1 with grafts, and with nothing else** (2026-07-20): stem children are the card's own body, not references, so they never consume rings. a branch card's shoot carries one ring per graft; a leaf card's shoot carries none. and since the graft count is derivable from the face, **replay can validate every shoot against its bloom's face** — ring-list length must equal graft count, a structural integrity check in the same family as the graft-petal-vs-child-schema check the encoding already promises
